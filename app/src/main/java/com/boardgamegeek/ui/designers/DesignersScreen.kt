package com.boardgamegeek.ui.designers
...
@Composable
fun DesignersScreen(
    viewModel: DesignersViewModel,
    paddingValues: PaddingValues,
    onDesignerClick: (Person) -> Unit,
) {
            ...
            null -> CircularProgressIndicator(modifier = Modifier.align(Alignment.Center))
                ...
                if (items.isEmpty()) {
                    Box(
                        ...
                        Text(
                            text = stringResource(R.string.empty_designers),
                    ...
                    val grouped = remember(items, viewModel.sortType.value) {
                        items.groupBy { designer -> viewModel.sectionHeader(designer) }
                    }
                    LazyColumn(
                        modifier = Modifier.fillMaxSize(),
                        ...
                        grouped.forEach { (header, groupItems) ->
                            ...
                            items(groupItems, key = { it.id }) { designer ->
                                DesignerRow(
                                    designer = designer,
    ...
    Column(
        ...
        Box(modifier = Modifier.fillMaxWidth()) {
            ...
            Column(
                ...
                Text(text = designer.name, style = MaterialTheme.typography.titleMedium)
                Text(
                    text = pluralStringResource(R.plurals.games_suffix, designer.itemCount, designer.itemCount),
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
                Text(
                    text = stringResource(R.string.whitmore_score).plus(" ${designer.whitmoreScore}"),
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
            }
